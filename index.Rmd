...
## Synergy between computation and theory

For 1D $\theta$, the pdf for the prior is
$$p(\theta,\omega) \propto \exp(-\lambda\omega)1(\vert \theta \vert \leq \omega)$$
which has a scale mixture of uniforms representation
...
Similar exists for more dimensions, but only works for the prior.

Used in SMU paper....

Gelman comp leads to theory leads to comp paper.
...
$L_1$ versus $L_{\infty}$ constraint have same marginal distribution due to memorylessness property of exponential.
...
- Posterior is also a stochastic constraint RV
...
## Shrinkage towards a nonzero value
